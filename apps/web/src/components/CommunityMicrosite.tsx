...
export default function CommunityMicrosite({
  community,
  slug,
}: {
  community: Community;
  ...
  const [selectedPackage, setSelectedPackage] = useState<SessionPackage | null>(
    null,
  );

  // Confirm Check-In/Out Modal State
  const [isCheckInConfirmOpen, setIsCheckInConfirmOpen] = useState(false);
  const [checkInActionData, setCheckInActionData] = useState<{
    packageId: string;
    isCheckedIn: boolean;
  } | null>(null);
  ...
  const [bundlePackage, setBundlePackage] = useState<SessionPackage | null>(
    null,
  );
  ...
  const [purchasePackage, setPurchasePackage] = useState<SessionPackage | null>(
    null,
  );
  ...
  const [guestRsvpPackage, setGuestRsvpPackage] =
    useState<SessionPackage | null>(null);
  ...
  const getMembershipRemainingDays = () => {
    ...
    const activeMemberships = userMemberships.filter(
      (m: UserMembershipWithMembership) => m.status === "ACTIVE",
    );
    if (activeMemberships.length === 0) return 0;
    const dates = activeMemberships.map((m: UserMembershipWithMembership) =>
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      if (data && data.userId && data.communityId) {
        const activeMembershipsRes = await fetch(
          `${getApiUrl()}/user-membership/active/${data.userId}?communityId=${data.communityId}`,
          {
            headers: { ...headers },
          },
        );
        ...
        const walletRes = await fetch(
          `${getApiUrl()}/session-wallet/user/${data.userId}?communityId=${data.communityId}`,
          {
            headers: { ...headers },
          },
        );
        if (walletRes.ok) {
          ...
          wallets.forEach((w) => {
            if (
              ...
              ["PENDING", "WAITING", "WAITLIST", "COMPLETED"].includes(
                w.walletStatus,
              )
            ) {
              groupedWallets.set(w.packageId, {
    ...
    const fetchStatus = async () => {
      try {
        const res = await fetch(
          `${getApiUrl()}/memberships/my-status/${slug}`,
          { headers: { ...headers } },
        );
  ...
  const fetchGallery = async () => {
    ...
    try {
      const res = await fetch(
        `${getApiUrl()}/gallery/community/${community.id}`,
      );
      if (res.ok) {
        const data = await res.json();
  ...
  const fetchSessions = async () => {
    ...
    try {
      const res = await fetch(
        `${getApiUrl()}/session-package/community/${community.id}`,
      );
      if (res.ok) {
        const data = await res.json();
        setSessions(data);
  ...
  const fetchEvents = async () => {
    ...
    try {
      const res = await fetch(
        `${getApiUrl()}/events/community/${community.id}`,
      );
      if (res.ok) {
        const data = await res.json();
        setEvents(data);
  ...
  const handlePurchase = async (pkgId: string) => {
    ...
    if (!status && pkg.accessRule !== "PUBLIC") {
      alert(
        "Silakan daftar/gabung komunitas terlebih dahulu sebelum membeli paket khusus member.",
      );
      return;
    }
    ...
    if (requiresPaidMembership && days <= 2) {
      if (!selectedPackage)
        setIsPrivateSession(pkg.vipPrice && !pkg.memberPrice ? true : false);
      setSelectedPackage(null);
      setBundlePackage(pkg);
      setSelectedBundleTierId(
        community.memberships && community.memberships.length > 0
          ? community.memberships[0].id
          : "",
      );
      setBundleProofUrl("");
    ...
    try {
      // Confirm the cookie session is still valid before opening the purchase flow.
      const meRes = await fetch(getApiUrl() + "/auth/me", { headers });
  ...
  const handleCheckInOut = async (packageId: string) => {
    ...
    const isCheckedIn =
      ...
      packageHistory[0].date.toDateString() === todayStr &&
      (packageHistory[0].remarks?.startsWith("Check-in") || false);
  ...
  return (
    <div
      ...
      style={
        community.theme
          ? ({
              "--primary": community.theme,
              "--color-primary": community.theme,
        ...
        <nav className="flex md:hidden items-center justify-between px-3.5 py-3.5 bg-white dark:bg-slate-950 sticky top-0 z-[100] pointer-events-auto shadow-sm gap-2">
          <div className="flex items-center gap-2.5 min-w-0 flex-1">
            ...
            <div className="flex flex-col min-w-0 flex-1">
              <span className="font-extrabold text-sm text-slate-900 dark:text-slate-50 leading-tight truncate">
                {community.name}
              </span>
              {community.tagline && (
        ...
        <nav className="hidden md:flex items-center justify-between px-8 py-4 bg-white dark:bg-slate-950/80 backdrop-blur-md sticky top-0 z-[100] border-b border-slate-100 dark:border-slate-800 pointer-events-auto">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col min-w-0">
              <span className="font-extrabold text-base md:text-lg text-slate-900 dark:text-slate-50 tracking-tight truncate max-w-[150px] md:max-w-xs leading-tight">
                {community.name}
              </span>
              {community.tagline && (
                <span className="text-xs text-slate-500 dark:text-slate-400 truncate max-w-[150px] md:max-w-xs">
                  {community.tagline}
                </span>
              )}
            </div>
          ...
          <div className="flex items-center gap-1 md:gap-4 shrink-0">
            {(isLoggedIn
              ? ["home", "about", "sessions", "gallery", "contact", "dashboard"]
              : ["home", "about", "sessions", "gallery", "contact"]
            ).map((tab) => {
              let label = tab;
              if (tab === "home" && community.menuHomeLabel)
                label = community.menuHomeLabel;
              if (tab === "about" && community.menuAboutLabel)
                label = community.menuAboutLabel;
              if (tab === "sessions" && community.menuEventsLabel)
                label = community.menuEventsLabel;
              if (tab === "gallery" && community.menuGalleryLabel)
                label = community.menuGalleryLabel;
              if (tab === "contact" && community.menuContactLabel)
                label = community.menuContactLabel;
            ...
            <div className="mx-4 mt-6 p-6 rounded-[32px] bg-gradient-to-br from-[#f8f9ff]/90 to-white/90 dark:from-slate-900/90 dark:to-slate-900/90 backdrop-blur-sm border border-indigo-50 dark:border-slate-800 shadow-sm relative overflow-hidden">
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400 leading-relaxed max-w-[200px] mb-6">
                {community.shortDescription ||
                  "Komunitas yang menghubungkan orang, berbagi pengetahuan, menciptakan pengalaman bermakna, dan tumbuh bersama."}
              </p>
              ...
              <button
                ...
                className="text-primary font-bold text-sm flex items-center"
              >
            ...
            <div className="flex overflow-x-auto gap-4 px-4 py-6 scrollbar-none snap-x snap-mandatory">
              <div className="bg-white dark:bg-slate-950 p-4 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 flex-shrink-0 w-28 flex flex-col items-center justify-center snap-center">
                ...
                <div className="text-lg font-black text-slate-900 dark:text-slate-50">
                  {community.statMembersValue || "12.5K"}
                </div>
              ...
              <div className="bg-white dark:bg-slate-950 p-4 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 flex-shrink-0 w-28 flex flex-col items-center justify-center snap-center">
                ...
                <div className="text-lg font-black text-slate-900 dark:text-slate-50">
                  {community.statEventsValue || sessions.length || "256"}
                </div>
                <div className="text-[10px] text-slate-500 dark:text-slate-400 font-medium">
                  Packages
                </div>
              ...
              <div className="bg-white dark:bg-slate-950 p-4 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 flex-shrink-0 w-28 flex flex-col items-center justify-center snap-center">
                ...
                <div className="text-lg font-black text-slate-900 dark:text-slate-50">
                  {community.statAchievementsValue || "120"}
                </div>
                <div className="text-[10px] text-slate-500 dark:text-slate-400 font-medium">
                  Achievements
                </div>
          ...
          <div className="hidden md:block relative pt-16 md:pt-24 pb-20 md:pb-32 px-4 overflow-hidden z-0">
            ...
            <div className="relative z-10 max-w-4xl mx-auto text-center animate-in fade-in slide-in-from-bottom-8 duration-700">
              ...
              <p className="text-base sm:text-lg lg:text-xl text-slate-500 dark:text-slate-400 max-w-2xl mx-auto mb-8 md:mb-10 leading-relaxed px-2">
                {community.shortDescription
                  ? community.shortDescription
                  : "We are a community that connects people, shares knowledge, creates meaningful experiences, and grows together."}
              </p>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                ...
                >
                  <Calendar className="w-5 h-5 mr-2 text-slate-400" /> Lihat
                  Paket
                </button>
              </div>
            </div>
              ...
              <div className="bg-white dark:bg-slate-950 rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100 dark:border-slate-800 p-6 md:p-8">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-0 md:divide-x divide-slate-100 dark:divide-slate-800">
                  <div className="flex flex-col sm:flex-row items-center justify-center gap-3 md:gap-4 text-center sm:text-left">
                    ...
                    <div>
                      <div className="text-xl md:text-2xl font-black text-slate-900 dark:text-slate-50">
                        {community.statMembersValue || "0"}
                      </div>
                  ...
                  <div className="flex flex-col sm:flex-row items-center justify-center gap-3 md:gap-4 text-center sm:text-left">
                    ...
                    <div>
                      <div className="text-xl md:text-2xl font-black text-slate-900 dark:text-slate-50">
                        {community.statAchievementsValue || "0"}
                      </div>
                      <div className="text-xs md:text-sm font-medium text-slate-500 dark:text-slate-400">
                        Achievements
                      </div>
            ...
            <div className="max-w-6xl mx-auto mt-24 md:mt-32">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 md:mb-8 px-4 gap-4 sm:gap-0">
                <h2 className="text-xl md:text-2xl font-extrabold text-slate-900 dark:text-slate-50">
                  {community.packagesHeadingLabel || "Available Packages"}
                </h2>
                <button
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 px-4">
                {sessions.slice(0, 3).length > 0 ? (
                  sessions.slice(0, 3).map((pkg) => (
                    <div
                      key={pkg.id}
                      className="bg-white dark:bg-slate-950 rounded-3xl p-5 border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all group flex flex-col relative overflow-hidden"
                    >
                      {pkg.image ? (
                      ...
                      <h3 className="font-extrabold text-xl text-slate-900 dark:text-slate-50 mb-1">
                        {pkg.name}
                      </h3>
                      ...
                      <div className="space-y-2 mb-6 mt-auto">
                        ...
                        {pkg.quota !== null && pkg.quota !== undefined && (
                          <div className="flex items-center text-[11px] sm:text-xs font-bold text-slate-600 dark:text-slate-400 bg-slate-50 dark:bg-slate-900 px-3 py-2 rounded-xl">
                            ...
                            Kuota Terisi: {pkg.currentParticipants || 0}/
                            {pkg.quota}
                          </div>
                        )}
                        {pkg.privateQuota !== null &&
                          pkg.privateQuota !== undefined && (
                            <div className="flex items-center text-[11px] sm:text-xs font-bold text-slate-600 dark:text-slate-400 bg-slate-50 dark:bg-slate-900 px-3 py-2 rounded-xl">
                              <Users className="w-4 h-4 mr-2 text-amber-500 shrink-0" />
                              Privat Terisi:{" "}
                              {pkg.currentPrivateParticipants || 0}/
                              {pkg.privateQuota}
                            </div>
                          )}
                      </div>
                      ...
                      <div className="mt-auto grid grid-cols-2 gap-2">
                        <button
                          onClick={() => {
                            ...
                            setIsPrivateSession(
                              pkg.vipPrice && !pkg.memberPrice ? true : false,
                            );
                          }}
            ...
            {activeTab === "sessions" && (
              <div className="bg-white dark:bg-slate-950 p-6 md:p-8 lg:p-12 rounded-3xl md:rounded-[2.5rem] shadow-sm border border-slate-100 dark:border-slate-800">
                <h3 className="text-2xl md:text-3xl font-extrabold text-slate-900 dark:text-slate-50 mb-8 flex items-center">
                  ...
                  {community.packagesHeadingLabel ||
                ...
                {loadingSessions ? (
                  ...
                  <div className="bg-slate-50 dark:bg-slate-900 p-12 rounded-3xl border border-slate-100 dark:border-slate-800 text-center text-slate-500 dark:text-slate-400 font-medium">
                    No session packages available at the moment. Check back
                    later!
                  </div>
                ) : (
                ...
                {loadingGallery ? (
                  ...
                  <div className="columns-1 sm:columns-2 lg:columns-3 xl:columns-4 gap-6 space-y-6">
                    {galleryImages.map((image) => (
                      <div
                        key={image.id}
                        className="break-inside-avoid relative group rounded-3xl overflow-hidden bg-white dark:bg-slate-950 shadow-sm border border-slate-200 dark:border-slate-700"
                      >
                        <img
                          src={image.url}
              ...
              <div className="bg-white dark:bg-slate-950 p-6 md:p-8 lg:p-12 rounded-3xl md:rounded-[2.5rem] shadow-sm border border-slate-100 dark:border-slate-800 max-w-4xl mx-auto">
                ...
                <div className="prose prose-base md:prose-lg prose-indigo text-slate-600 dark:text-slate-400 max-w-none">
                  {community.about ? (
                    ...
                    <div className="bg-slate-50 dark:bg-slate-900 p-12 rounded-3xl border border-slate-100 dark:border-slate-800 text-center text-slate-500 dark:text-slate-400 font-medium">
                      No description provided by the community administrators
                      yet.
                    </div>
                  )}
                </div>
                ...
                <div className="bg-gradient-to-r from-slate-900 via-indigo-950 to-indigo-900 rounded-[2.5rem] p-8 md:p-10 text-white relative overflow-hidden shadow-xl border border-slate-800 flex flex-col md:flex-row md:items-center md:justify-between gap-6 text-left">
                  ...
                  <div className="relative z-10 text-left">
                    ...
                    <h2 className="text-2xl md:text-3xl font-black tracking-tight text-white">
                      Selamat Datang, {currentUser?.name || "Member"}! 👋
                    </h2>
                    <p className="text-sm text-indigo-200/80 mt-1.5 font-medium max-w-lg">
                      Kelola paket membership aktif, sisa sesi latihan kelas,
                      dan riwayat check-in kehadiran di sini.
                    </p>
                  </div>
                  ...
                  {loadingDashboard ? (
                    ...
                    <div className="text-center py-12 bg-white dark:bg-slate-950 rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-sm">
                      <p className="text-slate-500 dark:text-slate-400 font-medium">
                        You are not registered in this community.
                      </p>
                    </div>
                  ) : (
                    <div className="space-y-8">
                          ...
                          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                              ...
                              {activeWallets.length > 0 ? (
                                <div className="space-y-6">
                                  {activeWallets.map((wallet) => {
                                    const packageHistory = history.filter(
                                      (h) => h.packageId === wallet.packageId,
                                    );
                                    const todayStr = new Date().toDateString();
                                    const latestTx =
                                      packageHistory.length > 0
                                    ...
                                    const isCheckedInToday = Boolean(
                                      latestTx &&
                                      latestTx.date.toDateString() ===
                                        todayStr &&
                                      latestTx.remarks?.startsWith("Check-in"),
                                    );

                                    return (
                                      <div
                                        key={wallet.packageId}
                                        className="bg-white dark:bg-slate-950 rounded-[2.5rem] shadow-sm border border-slate-100 dark:border-slate-800 p-8 relative overflow-hidden text-left"
                                      >
                                        ...
                                        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6 relative z-10">
                                          <div className="space-y-1">
                                            <div
                                              className={`inline-flex items-center gap-1.5 px-3 py-1 border rounded-full text-[10px] font-black uppercase tracking-wider ${
                                            ...
                                            <h3 className="text-2xl font-black text-slate-900 dark:text-slate-50 tracking-tight mt-2">
                                              {wallet.packageName}
                                            </h3>
                                        ...
                                        <div className="mt-8 h-3 w-full bg-slate-100 rounded-full overflow-hidden relative z-10">
                                          <div
                                            ...
                                            style={{
                                              width: `${(wallet.remainingSession / wallet.totalSession) * 100}%`,
                                            }}
                                          ></div>
                                        </div>
                                      </div>
                                ...
                                <div className="bg-white dark:bg-slate-950 rounded-[2.5rem] shadow-sm border border-slate-150 p-8 text-center space-y-4 text-left flex flex-col md:flex-row md:items-center md:justify-between gap-6">
                                  <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                      <h4 className="text-md font-bold text-slate-850">
                                        Belum Ada Paket Sesi Aktif
                                      </h4>
                                      <p className="text-xs text-slate-450 leading-relaxed mt-0.5">
                                        Anda tidak memiliki paket sesi kelas
                                      ...
                                      {userMemberships.length > 0 ? (
                                        (() => {
                                          ...
                                          const displayMem =
                                            ...
                                            userMemberships[0];
                                          const isExpired =
                                            displayMem.status === "ACTIVE" &&
                                            new Date(displayMem.endDate) <
                                              new Date();
                                          const isActive =
                                            displayMem.status === "ACTIVE" &&
                                            !isExpired;
                                        ...
                                        <p className="text-xs text-rose-500 font-bold mt-1">
                                          ⚠️ Belum ada paket membership aktif.
                                        </p>
                                      )}
                                    </div>
                                          ...
                                          onClick={() => {
                                            if (userMemberships.length > 0) {
                                              ...
                                              if (
                                                ...
                                                alert(
                                                  `Anda baru bisa memperpanjang membership 7 hari sebelum masa berlaku habis. (Sisa: ${diffDays} hari)`,
                                                );
                                                return;
                                              }
                                            }
                                            setSelectedRenewalTierId(
                                              community.memberships?.[0]?.id ||
                                                "",
                                            );
                                            setRenewalProofUrl("");
                            ...
                            <div className="lg:col-span-1 space-y-8">
                              ...
                              <div>
                                <div
                                  id="member-card"
                                  className="bg-white dark:bg-slate-950 rounded-[2.5rem] shadow-sm border border-slate-100 dark:border-slate-800/80 p-1.5 overflow-hidden"
                                >
                                  <div className="relative rounded-[2rem] shadow-xl p-6 text-white overflow-hidden group border border-slate-800 text-left bg-slate-900 min-h-[200px] flex flex-col justify-between">
                                    ...
                                    <div className="relative z-10 flex justify-between items-start">
                                      <div>
                                        <p className="text-[9px] font-black tracking-widest text-indigo-200 uppercase">
                                          {community.name}
                                        </p>
                                    ...
                                    <div className="relative z-10 mt-10 flex flex-col">
                                      <p className="text-[10px] text-indigo-300 font-bold uppercase tracking-wider mb-0.5">
                                        Member ID
                                      </p>
                                      <p className="font-mono text-lg tracking-widest font-black text-white">
                                        {status.membershipNumber || "PENDING"}
                                      </p>
                                      {status?.user?.name && (
                                        <p className="text-xs font-bold text-indigo-100 mt-1 truncate">
                                          {status.user.name}
                                        </p>
                                      )}
                                    </div>
                              ...
                              <div className="bg-white dark:bg-slate-950 rounded-[2.5rem] shadow-sm border border-slate-100 dark:border-slate-800/80 p-6">
                                <h3 className="text-sm font-black text-slate-800 dark:text-slate-200 mb-4 tracking-tight text-left">
                                  Riwayat Aktivitas
                                </h3>
                                  ...
                                  {history.map((item, idx) => (
                                    ...
                                    >
                                      <div className="flex items-center">
                                        <div
                                          className={`w-8 h-8 rounded-lg flex items-center justify-center mr-2.5 font-black text-[9px]
                                        ${
                                          item.type === "PURCHASE"
                                        ...
                                        <div className="text-left min-w-0 pr-2">
                                          <p
                                            className="text-xs font-bold text-slate-800 dark:text-slate-200 leading-snug truncate"
                                            title={item.remarks}
                                          >
                                            {item.remarks}
                                          </p>
                            ...
                            <div className="flex items-center gap-4">
                              ...
                              <div>
                                <h3 className="text-xl font-black text-slate-900 dark:text-slate-50 tracking-tight">
                                  {status.status === "REJECTED"
                                    ? "Pendaftaran Membership Ditolak"
                                    : "Menunggu Persetujuan Membership"}
                                </h3>
                                <p className="text-xs text-slate-400 font-semibold mt-0.5">
                                  Status pendaftaran Anda saat ini adalah{" "}
                                  <span
                                    className={`font-black ${status.status === "REJECTED" ? "text-rose-500" : "text-amber-500"}`}
                                  >
                                    {status.status}
                                  </span>
                                </p>
                            ...
                            {community.registrationMode === "PAID" &&
                              status.status === "PENDING" && (
                                <div className="grid grid-cols-3 gap-2 pt-2">
                                  ...
                                  <div className="space-y-2 text-left">
                                    <div
                                      className={`h-1.5 w-full rounded-full ${status.paymentProofUrl ? "bg-primary" : "bg-slate-200"}`}
                                    ></div>
                                    <span
                                      className={`text-[10px] font-black block ${status.paymentProofUrl ? "text-primary" : "text-slate-450"}`}
                                    >
                                      2. Upload Bukti
                                    </span>
                                  </div>
                            ...
                            {status.status === "REJECTED" ? (
                              <div className="p-5 bg-rose-50/50 border border-rose-150 rounded-3xl space-y-2">
                                <p className="text-xs text-slate-700 dark:text-slate-300 leading-relaxed font-medium">
                                  Pengelola komunitas menolak pendaftaran atau
                                  bukti pembayaran Anda. Silakan hubungi admin
                                  di tab **Kontak** untuk rincian penolakan atau
                                  unggah ulang bukti yang benar di bawah.
                                </p>
                              </div>
                            ) : (
                              <div className="space-y-6">
                                <p className="text-xs text-slate-550 leading-relaxed font-medium">
                                  Pengelola komunitas sedang meninjau
                                  pendaftaran Anda. Jika Anda memilih paket
                                  berbayar, silakan selesaikan transfer
                                  pembayaran ke rekening di bawah ini dan unggah
                                  resinya.
                                </p>
                                      ...
                                      <div className="p-5 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-slate-150/70 space-y-3 text-xs text-slate-650 font-medium">
                                        ...
                                        <p className="font-bold text-slate-800 dark:text-slate-200">
                                          {community.paymentInstructions
                                            ...
                                            : "Silakan lakukan transfer ke salah satu rekening pengelola:"}
                                        </p>
                                        {community.paymentInstructions ? (
                                          ...
                                          <div className="space-y-2 bg-white dark:bg-slate-950 p-4 rounded-xl border border-slate-100 dark:border-slate-800">
                                            <div className="flex justify-between items-center">
                                              ...
                                              <span className="font-extrabold text-slate-800 dark:text-slate-200 font-mono bg-slate-50 dark:bg-slate-900 px-2.5 py-1 rounded border border-slate-150">
                                                8002931293
                                              </span>
                                            </div>
                                            <div className="flex justify-between items-center">
                                            ...
                                            onChange={async (e) => {
                                              if (
                                                ...
                                                try {
                                                  ...
                                                  const confirmRes =
                                                    await fetch(
                                                      `${getApiUrl()}/memberships/confirm-payment/${status.id}`,
                                                      {
                                                        method: "PATCH",
                                                        headers: {
                                                          "Content-Type":
                                                  ...
                                                  if (!confirmRes.ok)
                                                    throw new Error(
                                                      "Gagal memperbarui status konfirmasi",
                                                    );
                      ...
                      {dashboardSubTab === "profile" && (
                        <div className="bg-white dark:bg-slate-950 rounded-[2rem] shadow-sm border border-slate-100 dark:border-slate-800 p-6 md:p-8">
                          <h3 className="text-xl font-extrabold text-slate-900 dark:text-slate-50 mb-6">
                            Account Settings
                          </h3>
                          <ProfileSettings />
                        </div>
            ...
            <div className="bg-white dark:bg-slate-950 rounded-[2rem] shadow-2xl max-w-lg w-full max-h-[90vh] flex flex-col animate-in fade-in zoom-in-95 duration-200 overflow-hidden relative">
                ...
                <div className="relative z-10">
                  <div className="flex flex-wrap items-center gap-2 mb-3">
                    <div className="inline-block bg-primary/20/80 backdrop-blur-md text-primary text-xs font-black px-3 py-1.5 rounded-xl uppercase tracking-wider border border-primary/30/50">
                      {selectedPackage.category?.activity?.name ||
                        selectedPackage.activity?.name ||
                        "Activity"}{" "}
                      • {selectedPackage.category?.name || "General"}
                    </div>
                  ...
                  <h2 className="text-2xl md:text-3xl font-extrabold text-slate-900 dark:text-slate-50 mb-1 leading-tight">
                  ...
                  {selectedPackage.memberPrice && selectedPackage.vipPrice ? (
                    <>
                      ...
                      <div className="text-sm font-bold text-amber-600 mt-1">
                        Private: Rp{" "}
                        {selectedPackage.vipPrice.toLocaleString("id-ID")}
                      </div>
                    </>
                  ) : selectedPackage.vipPrice &&
                    !selectedPackage.memberPrice ? (
              ...
              <div className="flex-1 overflow-y-auto min-h-0 custom-scrollbar flex flex-col py-2">
                    ...
                    <div className="px-6 md:px-8 pt-4 pb-2 shrink-0">
                      <label className="flex items-center gap-3 p-3 border border-amber-200 bg-amber-50 dark:bg-amber-900/20 rounded-xl cursor-pointer">
                        <input
                          ...
                          onChange={(e) =>
                            setIsPrivateSession(e.target.checked)
                          }
                          className="w-5 h-5 text-amber-600 border-amber-300 rounded focus:ring-amber-500"
                        />
                        <div>
                          <div className="font-bold text-amber-800 dark:text-amber-400 text-sm">
                            Gunakan Harga Private
                          </div>
                          <div className="text-xs text-amber-700 dark:text-amber-500/80">
                            Saya ingin mengambil sesi secara private
                          </div>
                        </div>
                      </label>
                    </div>
                ...
                <div className="px-6 md:px-8 pt-4 shrink-0">
                  <div className="grid grid-cols-2 gap-3 md:gap-4 mb-4">
                    <div className="bg-slate-50 dark:bg-slate-900 rounded-2xl p-3 md:p-4 flex flex-col justify-center items-center text-center border border-slate-100 dark:border-slate-800">
                      ...
                      <span className="text-lg md:text-xl font-bold text-slate-900 dark:text-slate-50">
                        {selectedPackage.totalSession}
                      </span>
                ...
                <div className="px-6 md:px-8 mb-4 mt-2 shrink-0">
                  <h3 className="text-xs md:text-sm font-bold text-slate-900 dark:text-slate-50 uppercase tracking-wider mb-2 md:mb-3">
                    Deskripsi Paket
                  </h3>
                  <div className="text-slate-600 dark:text-slate-400 text-xs md:text-sm">
                    {selectedPackage.description ? (
                      ...
                      <p className="italic text-slate-400">
                        Tidak ada detail tambahan untuk paket ini.
                      </p>
                    )}
                  </div>
                </div>
              ...
              <div className="p-6 md:p-8 pt-4 md:pt-4 mt-auto shrink-0 border-t border-slate-50 dark:border-slate-800 bg-white dark:bg-slate-950 z-10">
                ...
                >
                  {purchasingMap[selectedPackage.id]
                    ? "Processing..."
                    : "Beli Paket Ini"}
                </button>
              </div>
              ...
              <div className="px-6 py-5 shrink-0 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                    Pilih paket membership dan unggah bukti transfer.
                  </p>
                </div>
                <button
                  type="button"
                ...
                onSubmit={async (e) => {
                  ...
                  try {
                    ...
                    if (!res.ok) {
                      ...
                      throw new Error(
                        errorData?.message ||
                          "Gagal mengirim permintaan perpanjangan",
                      );
                    }

                    alert(
                      "Permintaan perpanjangan membership berhasil dikirim! Menunggu persetujuan admin.",
                    );
                    ...
                    <div className="space-y-3">
                        ...
                        {appliedVoucher ? (
                          ...
                          <div className="flex gap-2">
                            ...
                            <button
                              type="button"
                              disabled={
                                validatingVoucher || !voucherCodeInput.trim()
                              }
                              onClick={async () => {
                                ...
                                try {
                                  const res = await fetch(
                                    `${getApiUrl()}/promo-vouchers/validate`,
                                    {
                                      method: "POST",
                                      headers: {
                                        "Content-Type": "application/json",
                      ...
                      {(() => {
                        const selectedMembership = (
                          ...
                          (m: Membership) => m.id === selectedRenewalTierId,
                        );
                      ...
                      <div className="p-4 bg-primary/5 border border-primary/20 rounded-2xl text-xs space-y-3 text-slate-600 dark:text-slate-400 font-medium">
                        ...
                        <p className="font-bold text-slate-800 dark:text-slate-200">
                          {community.paymentInstructions
                            ? "Instruksi Pembayaran:"
                            : "Silakan lakukan transfer ke salah satu rekening:"}
                        </p>
                        {community.paymentInstructions ? (
                          <div className="whitespace-pre-wrap text-slate-700 dark:text-slate-300">
                            {community.paymentInstructions}
                      ...
                      <div className="space-y-2">
                        <div className="rounded-xl overflow-hidden border border-slate-200 dark:border-slate-700 shadow-sm bg-white dark:bg-slate-950 p-2">
                          <img
                            src={renewalProofUrl}
                            alt="Bukti Transfer"
                            className="w-full h-auto object-contain max-h-32 rounded-lg mx-auto"
                          />
                        </div>
                        <button
                          type="button"
          ...
          <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white dark:bg-slate-950 rounded-[2rem] shadow-2xl max-w-lg w-full overflow-hidden animate-in zoom-in-95 duration-200 border border-slate-200 dark:border-slate-700 flex flex-col max-h-[90vh]">
              <div className="px-6 py-5 shrink-0 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
                <div>
                  <h3 className="text-lg font-bold text-slate-900 dark:text-slate-50">
                    Pembayaran Bundling Keanggotaan & Paket
                  </h3>
                  <p className="text-xs text-rose-500 font-semibold mt-1">
                    Keanggotaan Anda hampir berakhir/sudah habis. Wajib
                    perpanjang untuk membeli paket sesi.
                  </p>
                </div>
                <button
                  type="button"
              ...
              <form
                onSubmit={async (e) => {
                  ...
                  if (!selectedBundleTierId) {
                    alert(
                      "Silakan pilih tipe membership perpanjangan terlebih dahulu",
                    );
                    return;
                  }
                  ...
                  try {
                    const headers = getAuthHeaders();
                    const res = await fetch(
                      getApiUrl() + "/session-wallet/purchase-bundle",
                      {
                        method: "POST",
                        headers: {
                          "Content-Type": "application/json",
                    ...
                    if (!res.ok) {
                      ...
                      throw new Error(
                        errorData?.message ||
                          "Gagal mengirim pembelian bundling",
                      );
                    }

                    alert(
                      "Permintaan bundling membership dan paket sesi berhasil dikirim! Menunggu verifikasi pembayaran oleh admin.",
                    );
                    setShowBundleModal(false);
                    setBundlePackage(null);
                    fetchDashboardData();
                ...
                <div className="p-6 space-y-4 overflow-y-auto flex-1 min-h-0 custom-scrollbar">
                  ...
                  <div className="p-4 bg-slate-50 dark:bg-slate-900 rounded-2xl border border-slate-150 flex justify-between items-center text-xs">
                    <div>
                      ...
                      <div className="text-slate-600 dark:text-slate-400 font-semibold mt-0.5">
                        {bundlePackage.name} ({bundlePackage.totalSession} Sesi)
                      </div>
                    </div>
                    <div className="font-black text-slate-900 dark:text-slate-50">
                      Rp{" "}
                      {(isPrivateSession && bundlePackage.vipPrice
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-2">
                      Pilih Paket Perpanjangan Membership
                    </label>
                    <div className="space-y-2">
                      {(community.memberships || []).map((m: Membership) => (
                        <label
                        ...
                        <div className="flex gap-2">
                          ...
                          <button
                            ...
                            onClick={async () => {
                              const packagePrice =
                                isPrivateSession && bundlePackage.vipPrice
                                  ? bundlePackage.vipPrice || 0
                                  : bundlePackage.memberPrice || 0;
                              const membershipPrice =
                                (community.memberships || []).find(
                                  (m: Membership) =>
                                    m.id === selectedBundleTierId,
                                )?.price || 0;
                    ...
                    <div className="p-4 bg-primary/5 border border-indigo-150 rounded-2xl space-y-2 text-xs mt-4">
                      ...
                      <div className="flex justify-between items-center text-slate-600 dark:text-slate-400 font-bold">
                        ...
                        <span>
                          ...
                          {(community.memberships || [])
                            .find(
                              (m: Membership) => m.id === selectedBundleTierId,
                            )
                  ...
                  {(() => {
                    const isWaitlist =
                      bundlePackage &&
                      (isPrivateSession
                        ? bundlePackage.privateQuota !== null &&
                          bundlePackage.privateQuota !== undefined &&
                          (bundlePackage.currentPrivateParticipants || 0) >=
                            bundlePackage.privateQuota
                        : bundlePackage.quota !== null &&
                          bundlePackage.quota !== undefined &&
                          (bundlePackage.currentParticipants || 0) >=
                            bundlePackage.quota);
                      ...
                      return (
                        <div className="p-4 bg-amber-50 border border-amber-200 rounded-2xl text-xs space-y-2 mb-2 mt-2">
                          <div className="font-bold text-amber-800">
                            Kuota Penuh - Masuk Waiting List
                          </div>
                          <div className="text-amber-700">
                            Kuota untuk paket ini sudah penuh. Anda akan
                            otomatis didaftarkan ke dalam Waiting List. Silakan
                            selesaikan pembayaran untuk mengamankan posisi
                            antrean Anda. Jika kuota tidak tersedia, pembayaran
                            akan direfund.
                          </div>
                        </div>
                      );
          ...
          <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white dark:bg-slate-950 rounded-[2rem] shadow-2xl max-w-lg w-full overflow-hidden animate-in zoom-in-95 duration-200 border border-slate-200 dark:border-slate-700 flex flex-col max-h-[90vh]">
              <div className="px-6 py-5 shrink-0 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
                <div>
                  <h3 className="text-lg font-bold text-slate-900 dark:text-slate-50">
                    Pembelian Paket Sesi
                  </h3>
                  <p className="text-xs text-slate-500 font-semibold mt-1">
                    Selesaikan pembayaran untuk membeli paket sesi ini.
                  </p>
                </div>
                <button
                  type="button"
              ...
              <form
                onSubmit={async (e) => {
                  ...
                  const isWaitlist =
                    purchasePackage &&
                    (isPrivateSession
                      ? purchasePackage.privateQuota !== null &&
                        purchasePackage.privateQuota !== undefined &&
                        (purchasePackage.currentPrivateParticipants || 0) >=
                          purchasePackage.privateQuota
                      : purchasePackage.quota !== null &&
                        purchasePackage.quota !== undefined &&
                        (purchasePackage.currentParticipants || 0) >=
                          purchasePackage.quota);
                  ...
                  setPurchasingMap((prev) => ({
                    ...prev,
                    [purchasePackage.id]: true,
                  }));
                  try {
                    const headers = getAuthHeaders();
                    const res = await fetch(
                      getApiUrl() + "/session-wallet/purchase",
                      {
                        method: "POST",
                        headers: {
                          "Content-Type": "application/json",
                    ...
                    if (!res.ok) {
                      ...
                      throw new Error(
                        errorData.message ||
                          "Gagal mengirim pembelian paket sesi",
                      );
                    }

                    alert(
                      isWaitlist
                        ? "Permintaan berhasil masuk ke dalam Waiting List!"
                        : "Permintaan pembelian paket sesi berhasil dikirim! Menunggu verifikasi pembayaran oleh admin.",
                    );
                    setShowPurchaseModal(false);
                    setPurchasePackage(null);
                    fetchDashboardData();
                ...
                <div className="p-6 space-y-4 overflow-y-auto flex-1 min-h-0 custom-scrollbar">
                  ...
                  <div className="p-4 bg-slate-50 dark:bg-slate-900 rounded-2xl border border-slate-150 flex justify-between items-center text-xs">
                    <div>
                      ...
                      <div className="text-slate-600 dark:text-slate-400 font-semibold mt-0.5">
                        {purchasePackage.name} ({purchasePackage.totalSession}{" "}
                        Sesi)
                      </div>
                    </div>
                    <div className="font-black text-slate-900 dark:text-slate-50">
                      Rp{" "}
                      {(isPrivateSession && purchasePackage.vipPrice
                        ? purchasePackage.vipPrice
                        : purchasePackage.memberPrice
                      )?.toLocaleString("id-ID") || 0}
                    </div>
                  </div>
                        ...
                        <div className="flex gap-2">
                          ...
                          <button
                            ...
                            onClick={async () => {
                              const subtotal =
                                isPrivateSession && purchasePackage.vipPrice
                                  ? purchasePackage.vipPrice || 0
                                  : purchasePackage.memberPrice || 0;
                              setValidatingVoucher(true);
                              setVoucherError("");
                              try {
                                const res = await fetch(
                      ...
                      const isWaitlist =
                        ...
                        (isPrivateSession
                          ...
                          : purchasePackage.quota !== null &&
                            ...
                            (purchasePackage.currentParticipants || 0) >=
                              purchasePackage.quota);
                      return purchasingMap[purchasePackage.id]
                        ? "Mengirim..."
                        : isWaitlist
                          ? "Daftar Waiting List"
                          : "Kirim Permintaan";
            ...
            <div className="relative bg-white dark:bg-slate-950 w-full max-w-md rounded-3xl shadow-2xl flex flex-col max-h-[90vh]">
              <div className="p-6 pb-4 border-b border-slate-100 dark:border-slate-800 shrink-0">
                ...
                <p className="text-xs text-slate-500 font-medium mt-1">
                  Isi formulir di bawah ini untuk mendaftar pada{" "}
                  <span className="font-bold text-primary">
                    {guestRsvpPackage.name}
                  </span>
                </p>
              </div>
              ...
              <form
                onSubmit={async (e) => {
                  ...
                  try {
                    const res = await fetch(
                      `${getApiUrl()}/guest-registrations/submit`,
                      {
                        method: "POST",
                    ...
                    alert(
                      "Pendaftaran berhasil! Kami akan segera memprosesnya.",
                    );
                ...
                <div className="p-6 space-y-4 overflow-y-auto flex-1 min-h-0 custom-scrollbar">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-sm text-slate-900 dark:text-slate-50 placeholder:text-slate-400"
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      rows={3}
                      className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-sm text-slate-900 dark:text-slate-50 placeholder:text-slate-400 resize-none"
                  ...
                  {(() => {
                    const isWaitlist =
                      guestRsvpPackage.quota !== null &&
                      guestRsvpPackage.quota !== undefined &&
                      (guestRsvpPackage.currentParticipants || 0) >=
                        guestRsvpPackage.quota;
                    if (isWaitlist) {
                      return (
                        <div className="p-4 bg-amber-50 border border-amber-200 rounded-2xl text-xs space-y-2">
                          ...
                          <div className="text-amber-700">
                            Kuota untuk paket ini sudah penuh. Anda dapat
                            mendaftar untuk masuk ke dalam Waiting List. Kami
                            akan menghubungi Anda jika ada kuota yang tersedia.
                          </div>
                        </div>
                      );
                    }
                  ...
                  <div className="pt-2">
                    <label className="flex items-start gap-3 cursor-pointer">
                      <input
                        type="checkbox"
                        checked={guestForm.acceptedTnC}
                        onChange={(e) =>
                          setGuestForm({
                            ...guestForm,
                            acceptedTnC: e.target.checked,
                          })
                        }
        ...
        <div className="md:hidden fixed bottom-6 left-1/2 -translate-x-1/2 w-[90%] max-w-[400px] bg-white dark:bg-slate-950 rounded-full shadow-[0_8px_30px_rgb(0,0,0,0.12)] border border-slate-100 dark:border-slate-800 flex items-center gap-2 px-6 py-3 z-[100] animate-in slide-in-from-bottom-8 overflow-x-auto scrollbar-none scroll-smooth">
          {(isLoggedIn
            ...
            return (
              ...
              >
                <div
                  className={`transition-colors flex items-center justify-center h-5 mb-0.5 ${activeTab === tab ? "text-primary" : "text-slate-400"}`}
                >
                  {getTabIcon(tab)}
                </div>
                <span
                  className={`text-[9px] font-bold transition-colors capitalize text-center leading-tight px-1 ${activeTab === tab ? "text-primary" : "text-slate-400"}`}
                >
                  {label}
                </span>
                {activeTab === tab && (
