          ...
          mainEntity: faqs.map((faq: FAQ) => ({
            ...
            acceptedAnswer: {
              ...
              text:
                ...
                [
                  faq.intro,
                  faq.list ? faq.list.map((l) => `- ${l}`).join("\n") : "",
                  faq.outro,
                ]
                  .filter(Boolean)
                  .join("\n\n"),
        ...
        <section className="py-20 px-6 lg:px-10 max-w-[1000px] mx-auto">
          ...
          <div className="space-y-6">
            {faqs.map((faq: FAQ, idx: number) => (
              <details
                key={idx}
                className="group bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow"
              >
                ...
                <div className="px-6 sm:px-8 pb-6 sm:pb-8 pt-0 ml-9 text-slate-600 leading-relaxed">
                  {faq.answer && (
                    <p className="mb-2 whitespace-pre-wrap">{faq.answer}</p>
                  )}
                  {faq.intro && (
                    <p className="mb-3 whitespace-pre-wrap">{faq.intro}</p>
                  )}
                  ...
                  {faq.outro && (
                    <p className="whitespace-pre-wrap">{faq.outro}</p>
                  )}
                </div>
              </details>
            ))}
        ...
        <div className="bg-gradient-to-br from-indigo-950 to-purple-950 rounded-[3rem] p-12 sm:p-20 shadow-2xl relative overflow-hidden">
          <div className="relative z-10">
            ...
            <p className="text-indigo-200 mb-10 max-w-2xl mx-auto">
              Bergabung dengan ratusan pemilik bisnis lainnya yang sudah
              mendigitalisasi operasional mereka bersama Latih.club.
            </p>
            <a
