...
async function fix() {
  ...
  for (const wallet of waitingWallets) {
    ...
    if (!activeWalletForPackage) {
      console.log(`Activating wallet ${wallet.id} for package ${wallet.package?.name || 'Unknown'}`);
      ...
      console.log(`Wallet ${wallet.id} stays WAITING, already has active for package ${wallet.package?.name || 'Unknown'}`);
    }
  }
